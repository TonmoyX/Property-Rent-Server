...
const JWKS = createRemoteJWKSet(new URL("https://localhost:300/api/auth/jwks"));
...
async function run() {
  try {
    ...
    const userFavouriteCollection = propsDB.collection("UserFavCollection");

    const usersRoleDB = client.db("PropRentClient");
    const usersCollection = usersRoleDB.collection("user");

    app.post("/addUserFav", async (req, res) => {
      const data = req.body;
      const result = await userFavouriteCollection.insertOne(data);
      res.json(result);
    });

    app.get('/addUserFav', async(req, res) => {
      const result = await userFavouriteCollection.find().toArray();
      res.json(result)
    });

     app.delete("/addUserFav/:id", async (req, res) => {
      const { id } = req.params;
      const query = { _id: new ObjectId(id) };
      const result = await userFavouriteCollection.deleteOne(query);
      res.json(result);
    });
